import type { Letter } from "./types.ts";


export const letters: Letter[] = [
 {
  subject: "Ja estàs dins. Ara la part incòmoda.",
  body: `Hola.

Acabes d’apuntar-te a Entreclases.

Abans de res, una aclaració: no t’he ficat en una newsletter. T’he ficat en una llista de gent que vol entrar.

T’escriuré set matins seguits, a les nou.

Set. No setanta.

I una cosa més, per estar ací abans que ningú: el dia que entres no comences amb les 20 ClasiCoins de tothom. Comences amb 50.

Encara no et diré per a què servixen.

Si et pareix massa, baix del tot tens l’enllaç per a donar-te de baixa. Un clic. Sense preguntes, sense enquesta d’eixida, sense «segur?».

Preferisc una llista xicoteta de gent que vol estar-hi, a una llista enorme de gent que m’esborra sense obrir.

Demà a les nou et conte per què vaig muntar això. Té a vore amb una taula de la cafeteria de Tarongers i amb com d’idiota em vaig sentir un dijous.

Silviu

P.D. Si el correu amb què t’has apuntat no és el de la teua universitat, tin a mà el bo. Només passen els comptes universitaris de València i qui porte una invitació d’algú de dins.`,
 },
 {
  subject: "Conec mitja uni. I quasi ningú.",
  body: `Tercer de carrera.

Em sabia la cara de dues-centes persones.

El nom d’onze.

Un dijous vaig eixir de classe, vaig baixar a la cafeteria i em vaig asseure sol. A dues taules, quatre de la meua facultat estaven muntant alguna cosa per al dissabte. Els sentia perfectament.

No em vaig girar.

Em vaig menjar l’entrepà mirant Instagram. Un desconegut a Bali. Una boda. Un altre de vint-i-dos anys explicant-me com ha guanyat el primer milió.

Estava envoltat de gent de la meua edat, del meu campus i de la meua carrera.

I el meu pla del dissabte era cap.

Això és el que vull arreglar. No amb més contingut. Amb un lloc on el normal siga tancar el mòbil i quedar amb algú.

Es diu Entreclases. Comença a les universitats de València.

Demà a les nou: per què això no és una altra xarxa social, encara que ho parega.

Silviu`,
 },
 {
  subject: "«Una altra xarxa social». Sí, ja.",
  body: `T’ho dic jo abans que ho penses tu.

Sí, és una altra app. I damunt la munta algú que es queixa de les xarxes socials. Molt coherent tot.

Però hi ha una diferència, i és l’única que m’importa.

Instagram guanya quan et quedes. Entreclases guanya quan te’n vas.

Si obris Entreclases, veus un pla, dius «m’apunte» i tanques el mòbil, ha funcionat.

Si et quedes quaranta minuts fent scroll, ha fallat.

Per això no hi ha mur infinit, ni comptes recomanats, ni un algoritme decidint què mereixes vore.

Hi ha gent del teu campus. Plans amb hora i lloc. Apunts. I un botó per a dir que hi vas.

Demà et conte el dels plans, que és el cor de tot això.

Silviu

P.D. No et promet vint amics nous. Et promet una excusa per a dir «hola» sense que siga rar. La resta la fas tu, que per a això ja eres major.`,
 },
 {
  subject: "Un café a Benimaclet a les sis",
  body: `El pla més avorrit del món:

«Café a Benimaclet. Dijous a les sis. Hi caben quatre.»

Ja està. Eixe és el producte.

No cal muntar un festival. No cal carisma. No cal una història per a contar.

Algú posa un lloc i una hora. Altres diuen que sí.

Perquè el difícil de la universitat mai va ser conéixer gent. El difícil era que algú fera el primer pas.

A Entreclases eixe pas és un botó.

Pots proposar el que vulgues: estudiar junts abans d’un parcial, una volta pel Túria, la Malva-rosa un dissabte, la posta de sol a l’Albufera.

O pots no proposar res i apuntar-te al que ja hi ha. També val. Quasi tothom comença així.

Demà: per què et demanaré el correu de la uni, i per què això és una bona notícia per a tu.

Silviu`,
 },
 {
  subject: "No és per a tothom. Eixa és la gràcia.",
  body: `Internet ja té un lloc on cap qualsevol.

Es diu internet.

Entreclases comença només amb correu universitari de València. Pública o privada, m’és igual, però d’ací.

Per què?

Perquè l’única cosa que convertix un desconegut en algú amb qui pots quedar és tindre alguna cosa en comú. Un campus. Una etapa. La possibilitat real de creuar-vos en eixir de classe.

Si òbric la porta a tothom, en una setmana això és Wallapop amb fotos de gent.

Ara, l’excepció. Cada compte universitari verificat pot convidar una persona de fora. Una. La teua parella, el teu amic del poble, la teua germana.

I qui entra convidat no pot convidar ningú més. Es talla ahí. Si no, la cadena es menja la idea en un mes.

Demà et parle de les ClasiCoins, que és la part que més em discuteixen.

Silviu`,
 },
 {
  subject: "Et cobraré per publicar",
  body: `Ací és on algú es dona de baixa. Endavant, sense rancor.

A Entreclases, crear un pla costa 10 ClasiCoins. Obrir un fil costa 5. El primer fil és gratuït.

Pregunta òbvia: em cobraràs diners?

No.

Les ClasiCoins no es compren, no es venen i no valen un euro. No hi ha botiga. No hi ha passarel·la de pagament. No existix el botó de «comprar monedes», i no existirà.

Comences amb 20 per completar el perfil.

Guanyes 2 per la teua primera resposta al fil d’una altra persona. Guanyes 3 la primera vegada que t’apuntes al pla d’algú.

I això per a què servix?

Perquè publicar coste alguna cosa. El que no costa res s’ompli de soroll. Mira el grup de WhatsApp de la teua classe i m’ho contes.

Si et quedes sense monedes, la manera d’aconseguir-ne més no és pagar. És ajudar algú.

Demà: el que ve després del 28.

Silviu`,
 },
 {
  subject: "El que encara no t’he contat",
  body: `Tinc una idea fixa.

A la teua mateixa facultat hi ha algú que programa, algú que dissenya i algú que sap vendre. I no es coneixeran mai, perquè mengen en cafeteries distintes i ningú els ha presentat.

Dos setmanes després d’obrir ix Projectes.

Funciona així: publiques la teua idea, dius quins llocs necessites i, molt important, què hi poses tu. Algú d’una altra carrera ho veu i t’envia una sol·licitud privada. Tu acceptes o no.

Hi ha fites, una conversa d’equip i, en acabar, un resultat amb els crèdits de tothom. Cadascú decidix si el mostra al seu perfil.

No és un tauler de «busque programador gratis per a la meua idea milionària». Has de dir què aportes tu.

I hi ha una tercera data de la qual encara no puc parlar. Està a la portada, per si la vols vore.

Demà l’últim de la sèrie. Et conte exactament què fer el dia que entres.

Silviu`,
 },
 {
  subject: "Això és el que fas el primer dia",
  body: `Últim correu de la sèrie. Gràcies per arribar fins ací, de veritat.

Això és el que fas el dia que entres, per ordre:

U. Entra amb el correu de la teua universitat. No el de Gmail. El de la uni.

Dos. Confirma el correu. T’arriba un enllaç, el polses, ja està.

Tres. Completa el perfil. Ahí apareixen les teues ClasiCoins, i si estaves en la llista abans que obrírem són 50 en lloc de 20.

Quatre, i este és el que importa de veritat: apunta’t al pla d’una altra persona el primer dia. No muntes el teu encara. Apunta’t al d’algú.

És molt més fàcil dir «hi vaig» que dir «veniu». Comença pel fàcil.

I si el correu amb què t’has apuntat a esta llista no és universitari, demana-li la invitació a algú de dins. Cada compte verificat pot portar una persona.

{{site}}

Ens veiem dins.

Silviu

P.D. T’ho dic clar: els primers dies són els que decidixen si un lloc així s’ompli o es queda en res. Un pla buit no convenç ningú. Preferisc que hi sigues.`,
 },
];
