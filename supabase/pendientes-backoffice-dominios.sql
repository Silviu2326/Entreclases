-- Entreclase: migraciones pendientes para backoffice y dominios de prueba.
-- Ejecutar solo si 202609210015 y 202609210016 no aparecen en supabase_migrations.schema_migrations.
-- Las cuentas de Auth deben crearse desde Supabase Auth; no se insertan contraseñas directamente en auth.users.
...
-- Concede admin a las cuentas cuando ya tengan usuario Auth y perfil Entreclase.
insert into public.universe_backoffice_roles (user_id, role)
select p.user_id, 'admin'
from public.universe_profiles p
join auth.users u on u.id = p.user_id
where lower(u.email) in ('silviu@opre.com', 'xarly@xarly.com')
on conflict (user_id) do update set role = excluded.role, revoked_at = null;

-- Comprobación final de dominios autorizados.
select domain, university_name, enabled, launch_region
from public.universe_university_domains
where domain in ('opre.com', 'xarly.com')
order by domain;
