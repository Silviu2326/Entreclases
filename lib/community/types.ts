...
import type { NoteStudy } from "./study";
import type { GameKind } from "./games/types";
import type { WarmupContent } from "./games/warmups";
export type View = "home" | "explore" | "projects" | "magazine" | "plans" | "groups" | "campus" | "people" | "messages" | "mailbox" | "profile" | "unicoins" | "discover" | "student";
export const views: View[] = ["home", "explore", "projects", "magazine", "plans", "groups", "campus", "people", "messages", "mailbox", "profile", "unicoins", "discover", "student"];
...
export interface CommunityRepository {
  ...
  readStudy(noteId: string): Promise<NoteStudy | null>;
  prepareStudy(noteId: string): Promise<NoteStudy>;
  askNote(noteId: string, question: string): Promise<string>;
  meetGamesOpen(): Promise<boolean>;
  warmups(game: GameKind): Promise<WarmupContent[]>;
