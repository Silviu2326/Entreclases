...
export const siteTitle = (locale: Locale) => locale === "va" ? "Entreclase — La teua universitat. La teua gent. València." : "Entreclase — Tu universidad. Tu gente. Valencia.";
...
export function gameMetadata(locale: Locale, demo: boolean, id: GameKind): Metadata {
 ...
 if (!game) return base;
 const title = `${game.title[languageIndex(locale)]} — Entreclase`, path = gamePath(locale, demo, id);
...
export function projectMetadata(locale: Locale, demo: boolean, section?: ProjectSection): Metadata {
 ...
 const title = `${entry ? entry.title[index] : locale === "va" ? "Projectes" : "Proyectos"} — Entreclase`;
