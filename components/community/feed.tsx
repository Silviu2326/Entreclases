...
import { threadCost } from "@/lib/community/unicoins";
...
export function Feed({compact=false,home=false,forum=false,search}:{compact?:boolean;home?:boolean;forum?:boolean;search?:string}={}){
 ...
 return <div className={`u-feed ${forum?"u-forum":""}`}>
  ...
  {posts.length?(home ? posts.slice(0, 6) : posts).map(p=><PostCard key={p.id} post={p} onCommunity={forum?pickCommunity:undefined}/>):<Empty title={selected?c("noCommunityPosts"):forum&&(topic!=="all"||filter==="groups")?c("noTopicPosts"):c("noPosts")} body={selected?c("noCommunityPostsBody"):forum&&(topic!=="all"||filter==="groups")?c("noTopicPostsBody"):c("noPostsBody")}/>}
 </div>;
}
