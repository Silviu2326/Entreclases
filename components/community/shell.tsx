...
import { Newspaper, ArrowUpRight, CalendarDays, ChevronRight, Compass, Home, MapPin, Mail, Menu, MessageCircle, Search, X, LogOut, Sparkles, ArrowRight, Settings2, VolumeX } from "lucide-react";
...
const motionPreferenceEvent="entreclases:motion-preference";
let memoryMotionPreference=false;
function getMotionPreference(){try{memoryMotionPreference=window.localStorage.getItem("entreclases:reduce-motion")==="true";}catch{/* Keep the current in-memory choice when storage is unavailable. */}return memoryMotionPreference;}
function subscribeMotionPreference(onChange:()=>void){window.addEventListener(motionPreferenceEvent,onChange);window.addEventListener("storage",onChange);return()=>{window.removeEventListener(motionPreferenceEvent,onChange);window.removeEventListener("storage",onChange);};}
function saveMotionPreference(value:boolean){memoryMotionPreference=value;try{window.localStorage.setItem("entreclases:reduce-motion",String(value));}catch{/* The preference still applies for this session. */}window.dispatchEvent(new Event(motionPreferenceEvent));}
...
export function CommunityShell({navigating=false,headingRef,query,setQuery,notice,dismissNotice}:{navigating?:boolean;headingRef:RefObject<HTMLHeadingElement|null>;query:string;setQuery:(q:string)=>void;notice:{text:string;error:boolean}|null;dismissNotice:()=>void;onRefresh:()=>Promise<void>}){
 ...
 const projectHref=(target:typeof locale)=>sectionEntry?projectPath(target,demo,sectionEntry.id,projectId??undefined):projectsPath(target,demo);const [drawer,setDrawer]=useState(false),[settingsOpen,setSettingsOpen]=useState(false),reduceMotion=useSyncExternalStore(subscribeMotionPreference,getMotionPreference,()=>false),[leaving,setLeaving]=useState(false),[logoutError,setLogoutError]=useState("");

 useEffect(()=>{document.documentElement.dataset.entreclasesMotion=reduceMotion?"reduced":"full";},[reduceMotion]);
 ...
 const nav=<><Link className="u-brand" href={localPath(locale,"home")}>entreclases<span aria-hidden="true">&#10035;</span></Link><span className="u-campus-stamp"><span/>Val&#232;ncia &#183; {c("campus")}</span><nav aria-label={c("navigation")}>{navigation.map(({view:v,Icon})=><button key={v} onClick={()=>navigate(v)} className={isCurrent(v)?"active":""} aria-current={isCurrent(v)?"page":undefined}><Icon aria-hidden="true"/>{c(v)}{isCurrent(v)&&<span className="u-nav-dot"/>}</button>)}</nav><div className="u-sidebar-preferences"><button type="button" className="u-settings-button" onClick={()=>setSettingsOpen(true)}><Settings2 aria-hidden="true"/><span>{locale==="va"?"Ajustos":"Ajustes"}</span></button></div>{!demo&&<div className="u-sidebar-bottom"><button className="u-logout" onClick={()=>void logout()} disabled={leaving}><LogOut aria-hidden="true"/>{c("logout")}</button>{logoutError&&<p role="alert" className="u-small">{logoutError}</p>}</div>}</>;

 const languageHref=(target:typeof locale)=>game?gamePath(target,demo,game):project?projectHref(target):localPath(target,demo?"demo":"app")+(view!=="home"?`?view=${view}`:"");
 ...
 return <><a href="#community-content" className="u-skip">{locale==="va"?"Anar al contingut":"Ir al contenido"}</a><aside className="u-sidebar">{nav}</aside><div className="u-workspace"><header className="u-topbar"><div className="u-mobile-brand"><IconAction label={c("more")} onClick={()=>setDrawer(true)}><Menu/></IconAction><Link className="u-brand" href={localPath(locale,"home")}>entreclases<span aria-hidden="true">&#10035;</span></Link></div><div className="u-breadcrumb"><span>Entreclases</span><ChevronRight aria-hidden="true"/>{gameEntry?<><button onClick={()=>go("explore")}>{c("explore")}</button><ChevronRight aria-hidden="true"/>{gameEntry.title[languageIndex(locale)]}</>:sectionEntry?<><Link href={projectsPath(locale,demo)}>{c("projects")}</Link><ChevronRight aria-hidden="true"/>{sectionEntry.title[languageIndex(locale)]}</>:c(view)}</div><div className="u-topbar-actions"><Mailbox/><CoinBalance/><div className="u-language" aria-label={locale==="va"?"Idioma":"Idioma"}>{(["es","va"] as const).map(l=><Link key={l} lang={l==="va"?"ca-ES-valencia":"es"} aria-current={l===locale?"page":undefined} href={languageHref(l)}>{l==="es"?"ES":"VAL"}</Link>)}</div><button className="u-top-avatar" onClick={()=>go("profile")} aria-label={c("profile")}><Avatar size="small" person={me}/></button></div>{navigating&&<RouteProgress view={view} label={locale==="va"?"Carregant la secció":"Cargando la sección"}/>}</header>{demo&&<div className="u-demo-banner"><Sparkles aria-hidden="true"/><p><strong>{c("demo")}.</strong> {c("demoHelp")}</p><Link href={localPath(locale,"login")}>{c("realAccess")}<ArrowUpRight aria-hidden="true"/></Link></div>}<div className={`u-page u-page-${view} ${view==="home"?"u-page-home":""}`}><main id="community-content" tabIndex={-1}><div className={`u-page-heading${view==="student"?" sr-only":""}`}><div><p className="u-eyebrow">{c(view)} &#183; {locale==="va"?"València":"Valencia"}</p><h1 ref={headingRef} tabIndex={-1}>{gameEntry?gameEntry.title[languageIndex(locale)]:sectionEntry?sectionEntry.title[languageIndex(locale)]:c(`${view}Title` as CopyKey)}</h1><p>{gameEntry?gameEntry.description[languageIndex(locale)]:sectionEntry?sectionEntry.body[languageIndex(locale)]:c(`${view}Sub` as CopyKey)}</p></div></div>{!game&&!sectionEntry&&view!=="student"&&<div className="u-search"><Search aria-hidden="true"/><label className="sr-only" htmlFor="community-search">{c("search")}</label><Input id="community-search" type="search" value={query} onChange={e=>setQuery(e.target.value)} placeholder={c("search")} autoComplete="off"/>{query&&<IconAction label={c("close")} onClick={()=>setQuery("")}><X/></IconAction>}</div>}<div className="u-screen" aria-busy={navigating} key={game??project??view}>{screen}</div><footer className="u-legal-footer"><LegalLinks locale={locale}/></footer></main>{view==="home"&&<RightRail/>}</div></div><nav className="u-bottom-nav" aria-label={c("navigation")}>{mobileNavigation.map(({view:v,Icon})=><button key={v} className={isCurrent(v)?"active":""} aria-current={isCurrent(v)?"page":undefined} onClick={()=>go(v)}><Icon aria-hidden="true"/><span>{c(v)}</span></button>)}</nav><Dialog.Root open={drawer} onOpenChange={setDrawer}><Dialog.Portal><Dialog.Overlay className="u-overlay"/><Dialog.Content className="u-mobile-drawer community-surface" aria-describedby={undefined}><Dialog.Title className="sr-only">{c("navigation")}</Dialog.Title><Dialog.Close asChild><IconAction label={c("close")}><X/></IconAction></Dialog.Close>{nav}</Dialog.Content></Dialog.Portal></Dialog.Root><Dialog.Root open={settingsOpen} onOpenChange={setSettingsOpen}><Dialog.Portal><Dialog.Overlay className="u-overlay u-settings-overlay"/><Dialog.Content className="u-settings-dialog community-surface" aria-describedby="u-settings-description"><header className="u-settings-heading"><div><p className="u-eyebrow">{locale==="va"?"EL TEU ESPAI":"TU ESPACIO"}</p><Dialog.Title>{locale==="va"?"Ajustos":"Ajustes"}</Dialog.Title><Dialog.Description id="u-settings-description">{locale==="va"?"Tria com vols vore i recórrer Entreclases.":"Elige cómo quieres ver y recorrer Entreclases."}</Dialog.Description></div><Dialog.Close asChild><IconAction label={locale==="va"?"Tancar ajustos":"Cerrar ajustes"}><X/></IconAction></Dialog.Close></header><section className="u-settings-section" aria-labelledby="u-settings-language"><div><h3 id="u-settings-language">{locale==="va"?"Idioma":"Idioma"}</h3><p>{locale==="va"?"Canvia l’idioma de tota la plataforma.":"Cambia el idioma de toda la plataforma."}</p></div><div className="u-settings-languages" role="group" aria-label={locale==="va"?"Idioma de la plataforma":"Idioma de la plataforma"}>{(["es","va"] as const).map(l=><Link key={l} lang={l==="va"?"ca-ES-valencia":"es"} aria-current={l===locale?"page":undefined} className={l===locale?"active":""} href={languageHref(l)}>{l==="es"?"Español":"Valencià"}</Link>)}</div></section><section className="u-settings-section u-settings-motion"><div className="u-settings-setting-icon"><VolumeX aria-hidden="true"/></div><div className="u-settings-motion-copy"><h3>{locale==="va"?"Reduir moviment":"Reducir movimiento"}</h3><p>{locale==="va"?"Limita animacions i transicions de la interfície.":"Limita las animaciones y transiciones de la interfaz."}</p></div><label className="u-settings-switch"><span className="sr-only">{locale==="va"?"Reduir moviment":"Reducir movimiento"}</span><input type="checkbox" checked={reduceMotion} onChange={event=>saveMotionPreference(event.target.checked)}/><span aria-hidden="true"/></label></section></Dialog.Content></Dialog.Portal></Dialog.Root>{notice&&<div className={`u-toast community-surface ${notice.error?"error":""}`} role={notice.error?"alert":"status"}><p>{notice.text}</p><IconAction label={c("close")} onClick={dismissNotice}><X/></IconAction></div>}</>;
