...
export function BackofficeApp({ locale = "es" }: { locale?: "es" | "va" }) {
  ...
  const text = locale === "va" ? { demo: "Vista de demostració", title: "El que necessita atenció.", sub: "Gestiona Entreclase amb context i sense perdre el fil.", search: "Buscar en el backoffice…" } : { demo: "Vista de demostración", title: "Lo que necesita atención.", sub: "Gestiona Entreclase con contexto y sin perder el hilo.", search: "Buscar en el backoffice…" };
  ...
  return <div className="bo-app">
    <aside className="bo-sidebar">
      <a className="bo-brand" href="/demo/?view=explore"><span className="bo-brand-mark">E</span><span>entreclase</span></a>
      <div className="bo-workspace"><span className="bo-workspace-dot" />Equipo Entreclase <small>València</small></div>
...
function Editorial({ items, onState, busyAction }: { items: Proposal[]; onState: (id: string | number, state: Exclude<Proposal["state"], "En revisión">) => void; busyAction?: string | null }) { return <section className="bo-detail"><div className="bo-detail-intro"><span className="bo-eyebrow">REDACCIÓN</span><h2>Lo que autorizan, se publica.</h2><p>Revisa cada propuesta y conserva la versión que su autor ha aprobado.</p></div><div className="bo-edition-strip"><span><Newspaper size={17} />Edición 03</span><strong>Esta semana en Entreclase</strong><small>Publicación prevista · jueves 24</small></div><div className="bo-list">{items.length ? items.map(item => <ProposalRow key={item.id} item={item} onState={onState} busyAction={busyAction} />) : <EmptyState text="No hay propuestas que coincidan con la búsqueda." />}</div></section>; }
function People({ query }: { query: string }) { const people = [{ name: "Paula Martí", detail: "Diseño industrial · Tarongers", state: "Activa", initials: "PM" }, { name: "Marc Ferrer", detail: "Derecho · Tarongers", state: "Activa", initials: "MF" }, { name: "Laia Soler", detail: "Psicología · Blasco Ibáñez", state: "Revisión pendiente", initials: "LS" }, { name: "Nico Vidal", detail: "Ingeniería informática · Vera", state: "Activa", initials: "NV" }].filter(person => `${person.name} ${person.detail}`.toLowerCase().includes(query.toLowerCase())); return <section className="bo-detail"><div className="bo-detail-intro"><span className="bo-eyebrow">CUENTAS</span><h2>Gente con una cuenta aquí.</h2><p>Acceso, verificación y señales de cuidado de la comunidad.</p></div><div className="bo-people-grid">{people.map(person => <article className="bo-person-card" key={person.name}><span className="bo-person-avatar">{person.initials}</span><div><strong>{person.name}</strong><small>{person.detail}</small><span className={person.state === "Activa" ? "bo-person-live" : "bo-person-review"}>{person.state}</span></div><button aria-label={`Abrir ${person.name}`}><ArrowUpRight size={16} /></button></article>)}</div></section>; }
