...
function SectionPlaceholder({ section }: { section: BackofficeSection }) {
  const copy: Record<BackofficeSection, { title: string; body: string; items: string[] }> = {
    overview: { title: "Resumen", body: "La actividad de Entreclases, lista para tu siguiente decisión.", items: [] },
...
export function Backoffice() {
  ...
  return <div className="bo-app">
    <aside className={`bo-sidebar ${menuOpen ? "is-open" : ""}`}><div className="bo-brand"><span className="bo-brand-glyph">e</span><span>entreclases</span></div><div className="bo-side-context"><i />Panel interno <span>València</span></div><nav aria-label="Secciones del backoffice"><p className="bo-nav-label">GESTIONAR</p>{navItems.map(item => { const Icon = iconBySection[item.id]; return <button key={item.id} type="button" className={section === item.id ? "is-active" : ""} onClick={() => chooseSection(item.id)}><Icon aria-hidden="true" /><span>{item.label}</span>{item.note && <small>{item.note}</small>}</button>; })}</nav><div className="bo-sidebar-foot"><div className="bo-admin-avatar">AT</div><div><strong>Álex Torres</strong><span>Administrador</span></div><button type="button" aria-label="Abrir configuración" onClick={() => chooseSection("settings")}><Settings2 aria-hidden="true" /></button></div></aside>
    <main className="bo-main"><header className="bo-topbar"><button type="button" className="bo-menu-button" aria-label="Abrir menú" onClick={() => setMenuOpen(value => !value)}><Menu /></button><div className="bo-breadcrumb"><span>Entreclases</span><ChevronRight aria-hidden="true" /><strong>Backoffice</strong></div><div className="bo-top-actions"><label className="bo-search"><Search aria-hidden="true" /><input value={query} onChange={event => setQuery(event.target.value)} placeholder="Buscar en el panel" aria-label="Buscar en el panel" /></label><button type="button" className="bo-notification" aria-label="Notificaciones"><Bell /><i>3</i></button><span className="bo-top-avatar">AT</span></div></header><div className="bo-content">{section === "overview" ? <Overview selected={selected} setSelected={setSelected} /> : <SectionPlaceholder section={section} />}</div></main>
