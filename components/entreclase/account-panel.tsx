...
export function AccountPanel({ locale = "es" }: { locale?: Locale }) {
  ...
  return <><p className="eyebrow">{tr("Tu cuenta de Entreclase")}</p><h1>{member ? <>{tr("Ya tienes sitio,")}<br />{member.name || tr("colega")}.</> : <>{tr("Tu sitio")}<br />{tr("empieza aquí.")}</>}</h1>{member ? <><p className="auth-intro">{tr("Correo confirmado. Lo próximo empieza con un hola.")}</p><div className="account-details"><p><BadgeCheck aria-hidden="true" /><strong>{tr("Correo universitario verificado")}</strong></p><p><Mail aria-hidden="true" /><span>{member.email}</span></p><p><School aria-hidden="true" /><span>{member.university}</span></p></div><Button asChild className="entreclase-button auth-submit"><Link href={localHref(locale,"/app/")}>{tr("Entrar a mi campus")}</Link></Button><Button className="entreclase-button auth-submit" onClick={logout} disabled={pending}><LogOut aria-hidden="true" />{pending ? tr("Cerrando…") : tr("Cerrar sesión")}</Button></> : <><OpeningNotice locale={locale} />{!error ? <p role="status" className="auth-intro">{tr("Comprobando tu cuenta…")}</p> : <p className="auth-intro">{tr("Inicia sesión con tu correo universitario para ver tu cuenta.")}</p>}<Button asChild className="entreclase-button auth-submit"><Link href={localHref(locale, "/login/")}>{tr("Iniciar sesión")}</Link></Button></>}<AuthError message={error} locale={locale} /><p className="auth-switch"><Link href={localHref(locale, "/")}>{tr("Volver a Entreclase")}</Link></p></>;
}
