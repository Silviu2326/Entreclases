...
10. Antes de activar «Estudiar con estos apuntes» y los calentamientos (migraciones 029 y 032, funciones `note-study` y `warmup-writer`): firmar el acuerdo de tratamiento de datos de OpenAI, decidir si se usa su residencia de datos en la UE (`OPENAI_BASE_URL=https://eu.api.openai.com/v1` con un proyecto configurado para la UE) y revisar el párrafo de privacidad que describe este uso. Las condiciones deberían recordar que solo se comparten apuntes propios o con permiso: la IA procesa el texto completo del PDF.
