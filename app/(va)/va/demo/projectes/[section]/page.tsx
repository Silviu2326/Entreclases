...
export async function generateMetadata({ params }: Params) { const section = sectionBySlug("va", (await params).section); return projectMetadata("va", true, section?.id); }
export default async function Page({ params }: Params) { const section = sectionBySlug("va", (await params).section); return <CommunityApp locale="va" demo project={section?.id ?? "hub"} />; }
