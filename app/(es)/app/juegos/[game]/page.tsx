...
export async function generateMetadata({ params }: Params) { const game = gameBySlug("es", (await params).game); return game ? gameMetadata("es", false, game.id) : {}; }
export default async function Page({ params }: Params) { const game = gameBySlug("es", (await params).game); return <CommunityApp locale="es" game={game?.id ?? null} />; }
