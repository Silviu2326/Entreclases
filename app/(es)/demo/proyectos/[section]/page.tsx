...
export async function generateMetadata({ params }: Params) { const section = sectionBySlug("es", (await params).section); return projectMetadata("es", true, section?.id); }
export default async function Page({ params }: Params) { const section = sectionBySlug("es", (await params).section); return <CommunityApp locale="es" demo project={section?.id ?? "hub"} />; }
